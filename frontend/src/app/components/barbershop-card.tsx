      ...
      <div className="relative h-48 overflow-hidden bg-muted">
          ...
          src={
            barbershop.images && barbershop.images.length > 0
              ? `${import.meta.env.VITE_API_URL}${
                  barbershop.images.find((img) => img.is_primary)?.image_url ||
                  barbershop.images[0].image_url
                }`
        ...
        <div className="absolute top-4 right-4 bg-card/95 backdrop-blur-sm px-2.5 py-1 rounded-lg flex items-center gap-1 border border-border shadow-sm">
          ...
          <span className="font-bold text-card-foreground text-xs">
            {barbershop.ratings_avg_rating
              ? parseFloat(String(barbershop.ratings_avg_rating)).toFixed(1)
              : "New"}
          </span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-1.5 text-muted-foreground mb-4">
          ...
          <span className="text-xs font-light">
            {barbershop.open_time
              ? String(barbershop.open_time).substring(0, 5)
              : "09:00"}{" "}
            -{" "}
            {barbershop.close_time
              ? String(barbershop.close_time).substring(0, 5)
              : "21:00"}
          </span>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-[10px] text-muted-foreground font-light uppercase leading-none mb-1">
              Starts from
            </p>
