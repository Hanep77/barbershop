...
export const register = async (name: string, email: string, role: string, password: string, password_confirmation: string) => {
  await getCsrf();
  return api.post('/api/register', { name, email, role, password, password_confirmation });
};
